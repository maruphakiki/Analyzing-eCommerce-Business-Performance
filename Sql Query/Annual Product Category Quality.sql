...
CREATE TABLE top_product_category AS
  ...
  FROM (
  	SELECT
  		...
  		RANK() OVER (PARTITION BY date_part('year', shipping_limit_date)         -- Membuat sistem ranking berdasarkan jumlah pendapatan tiap tahun
...
CREATE TABLE most_canceled_category AS
  ...
  FROM (
  	SELECT
  		...
  		RANK() OVER (PARTITION BY date_part('year', shipping_limit_date) -- Membuat sistem ranking berdasarkan jumlah cancel terbanyak tiap tahun
...
-- Menghapus anomali data tahun 2020
