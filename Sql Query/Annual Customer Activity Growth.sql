...
SELECT year, ROUND(AVG(customer_total),2) AS avg_mau
FROM (
  SELECT 
...
FROM (
  SELECT
  		Min(date_part('year', od.order_purchase_timestamp)) AS year, --ekstraksi tahun customer melakukan pembelian pertama kali (min = first buy)
...
FROM (
  ...
  JOIN customers_dataset AS cd
  	ON cd.customer_id = od.customer_id
  GROUP BY 1, 2
  HAVING count(3) > 1 --klausa repeat order
  ) AS sub
GROUP BY 1
ORDER BY 1
;
...
--Membuat Tabel untuk keperluan visualisasi
CREATE TABLE hasil_analisis (
  year integer,
  avg_mau numeric(10,2),
  total_new_customer integer,
  total_customer_repeat integer,
  avg_frequency numeric(10,3)
);
